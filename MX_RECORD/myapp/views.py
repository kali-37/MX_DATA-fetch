    ...
    if request.method == "POST":
        ...
        if not all(elem in df.columns.tolist() for elem in required_cols):
            return render(
                ...
                "import_file.html",
                {
                    "error": "Invalid file format. The file should have the following row at top: S. NO., COMPANY NAME, CONTACT PERSON, ADD., CITY, PIN, COUNTRY, STATE, LANDLINE NO., MOBILE NO., EMAIL ID, WEBSITE"
                },
            )
        ...
        for index, row in df.iterrows():
            website = row[
                "WEBSITE"
            ]  # row['website'] is the website column in the CSV/XLSX file
            website = (
                (
                    re.search(
                        r"(?:https?://)?(?:www\.)?([\w-]+\.[\w.-]+)", str(website)[:254]
                    ).group(1)
                )
                if re.search(
                    r"(?:https?://)?(?:www\.)?([\w-]+\.[\w.-]+)", str(website)[:254]
        ...
        if companies:
            try:
                ...
                return render(
                    ...
                    "import_file.html",
                    {
                        "error": "Another user just updated similar type of data in database. Please  reload the page after some time."
                    },
                )
...
def LoginPage(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            )  # html file ma jada form pass karna hai toh dictionary mei pass karo
    else:
...
def search_results(request):
    ...
    total_amount_of_new_unverified_data = UnverifiedData.objects.exclude(
        s_no__in=[-1, -2, -3]
    ).count()
    dashboard_data = {
    ...
    if request.method == "POST" and request.POST.get("domain") is not None:
        ...
        mail_servers = MailServerHistorical.objects.filter(
            mx_record__in=mx_records
        ).prefetch_related("mx_record__domain")
        context = {
    ...
    if (
        request.method == "POST"
        and request.POST.get("month") is not None
        and request.POST.get("country") is not None
        and request.POST.get("state") is not None
    ):
        ...
        mail_servers = MailServercurrent.objects.filter(
            current_mx__in=mx_records
        ).prefetch_related("current_mx__domain")
        context = {
            "mx_records": mx_records,
    ...
    if (
        request.method == "GET"
        and request.GET.get("export") == "csv"
        and request.GET.get("selected_month") is not None
        and request.GET.get("selected_country") is not None
        and request.GET.get("selected_state") is not None
    ):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="mx_records_{selected_country}_{selected_state}_{selected_month}.csv"'
        )
    ...
    return render(
        request,
        "search_results.html",
        {"countries": countries, "states": states, "dashboard_data": dashboard_data},
    )
...
def delete_all_unverified_data(request):
    if request.method == "POST":
        ...
        return render(
            ...
            "delete.html",
            {
                "message": "All unverified data deleted successfully.",
                "dashboard_data": dashboard_data,
            },
        )
...
def delete_all_domain_not_live_data(request):
    if request.method == "POST":
        ...
        return render(
            ...
            "delete.html",
            {
                "message": "All domain not live data deleted successfully from unverified database.",
                "dashboard_data": dashboard_data,
            },
        )
...
def delete_all_domain_live_no_mx_data(request):
    if request.method == "POST":
        ...
        return render(
            ...
            "delete.html",
            {
                "message": "All domain live but MX record not found data deleted successfully from unverified database.",
                "dashboard_data": dashboard_data,
            },
        )
...
def delete_all_verified_data(request):
    if request.method == "POST":
        ...
        if confirm == "yes":
            ...
            total_domain_not_live = UnverifiedData.objects.filter(s_no=-1).count()
            total_domain_live_with_no_MX = UnverifiedData.objects.filter(
                s_no=-2
            ).count()
            total_amount_of_mx_data = Domain.objects.all().count()
            total_amount_of_new_unverified_data = UnverifiedData.objects.exclude(
            ...
            return render(
                ...
                "delete.html",
                {
                    "message": "All verified data deleted successfully from domain database.",
                    "dashboard_data": dashboard_data,
                },
            )
        else:
            return render(
                ...
                "delete.html",
                {
                    "message": "Deletion of verified data canceled.",
                    "dashboard_data": dashboard_data,
                },
            )
