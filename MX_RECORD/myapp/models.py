...
class UnverifiedData(models.Model):
    ...
    website = models.CharField(max_length=254, primary_key=True, db_collation='utf8mb4_bin')
    def __str__(self):
...
class MXRecordcurrent(models.Model):
    domain = models.ForeignKey(Domain, on_delete=models.CASCADE, related_name='mx_records_current') 
    first_seen = models.DateField()
    last_seen = models.DateField()
    organizations = models.TextField()
    selected_month=models.CharField(max_length=70,default='')
...
class MailServerHistorical(models.Model):
    mx_record = models.ForeignKey(MXRecordAll, on_delete=models.CASCADE, related_name='mail_servers') # 
...
class MailServercurrent(models.Model):
    current_mx= models.ForeignKey(MXRecordcurrent, on_delete=models.CASCADE, related_name='current_mail_servers') # 
